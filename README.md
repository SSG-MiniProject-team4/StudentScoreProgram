...
| `Student.java`            | 학생 정보 모델 클래스. 이름, 점수, 총점, 평균, 학점 속성 포함.                      |
| `StudentComparator.java`  | 정렬 기준 정의 클래스.                                                |
| `ErrorCode.java` / `StudentText.java` | 상수 관리 열거형 클래스. 오류 메시지(`ErrorCode`)와 안내 문구(`StudentText`) 정의  |
...
- `main()` 메서드에서 다음 클래스들을 순서대로 실행:
    ...
    - `StudentOutput`
    - `SortedStudent`
- 사용자 입력 → 성적 처리 → 파일 출력 → 정렬 및 저장까지 **전체 흐름 제어**
...
- `Serializable` 인터페이스 구현 → **객체 데이터 파일 저장 및 불러오기 가능**
